...
void DetectorConstruction::ConstructECCFilms(G4LogicalVolume* motherLogical) {
    ...
    G4Material *soil = new G4Material("soil", 2.56 * g/cm3, 8); //taken from engineeringtoolbox soil-earth weight
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (i< 9) {
            ...
            G4PVPlacement* eccPlacement3 = new G4PVPlacement(0, G4ThreeVector(0, 0, ecc_sizeZ + plastic_sizeZ + ecc_positionZ + i * (2 * ecc_sizeZ + plastic_sizeZ)), eccLogical3, "ECC3", motherLogical, false, 0);
        } else {
            G4Box* eccSolid = new G4Box("ECC", ecc_sizeX, ecc_sizeY, ecc_sizeZ);
            G4LogicalVolume* eccLogical = new G4LogicalVolume(eccSolid, emulsion, "ECC");
            G4PVPlacement* eccPlacement = new G4PVPlacement(0, G4ThreeVector(0, 0, ecc_positionZ + i * (2 * ecc_sizeZ + plastic_sizeZ)), eccLogical, "ECC", motherLogical, false, 0);
    ...
    for (int i = 0; i < 10; ++i) {
        if (i == 0) {
            ...
            // ECC film
            G4Box* eccSolid5 = new G4Box("ECC5", ecc_sizeX, ecc_sizeY, ecc_sizeZ);
            G4LogicalVolume* eccLogical5 = new G4LogicalVolume(eccSolid5, emulsion, "ECC5");
            G4PVPlacement* eccPlacement5 = new G4PVPlacement(0, G4ThreeVector(0, 0, soil_sizeZ + soil_positionZ + ecc_positionZ + i * (2 * ecc_sizeZ + plastic_sizeZ)), eccLogical5, "ECC5", motherLogical, false, 0);
